...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Total nilai transaksi harian} (nominal rupiah dari \textit{invoice} lunas).
        \item \textit{Jumlah transaksi per hari} (volume \textit{invoice} harian).
        \item \textit{Jumlah produk terjual per hari} (kuantitas unit barang terjual).
        \item \textit{Indeks hari dalam seminggu} (fitur \textit{Day of Week} berupa indeks \textit{integer} 0 s.d. 4 untuk Senin--Jumat, dan 5--6 untuk Sabtu--Minggu).
        \item \textit{Status libur akademis} (fitur biner \textit{Is\_Holiday}, bernilai 1 jika berada di bulan libur semester/Ramadan, dan 0 jika masa perkuliahan aktif).
    \end{itemize}
    \item \textbf{Variabel \textit{Output} (Target):} Nilai prediksi total nominal penjualan harian untuk hari berikutnya, yang digunakan sebagai dasar pengambilan keputusan manajemen stok (terutama produk makanan basah) dan strategi operasional usaha.
...
Gambar \ref{fig:lstm_alur} menggambarkan alur kerja model LSTM dari \textit{input} data transaksi harian hingga proses evaluasi model. Adapun langkah-langkah proses algoritma dalam penelitian ini adalah sebagai berikut:
\begin{enumerate}
    ...
    \item Data disusun dalam bentuk deret waktu melalui proses \textit{windowing} (\textit{time step}) untuk membentuk pola historis yang digunakan sebagai \textit{input} model LSTM.
    \item \textit{Dataset} dibagi menjadi data \textit{training} dan \textit{testing} menggunakan metode \textit{time-based split} untuk menjaga urutan temporal data.
    ...
    \item Model prediksi yang telah teruji diintegrasikan ke dalam sistem POS melalui \textit{REST API} sehingga aplikasi \textit{mobile} dapat menampilkan hasil prediksi penjualan harian kepada pengguna.
\end{enumerate}
