...
Ketidakakuratan estimasi permintaan harian berdampak langsung pada efisiensi biaya dan tingkat layanan. Peramalan penjualan yang akurat berperan penting dalam menyeimbangkan permintaan dan pasokan serta meningkatkan profitabilitas pada sektor ritel~[36]. Karakteristik data penjualan harian pada usaha kuliner institusional bersifat kompleks: terdapat musiman mingguan berupa perbedaan volume antara hari kerja dan akhir pekan, pengaruh kalender akademik, hari libur nasional, serta fluktuasi yang tidak beraturan. Metode statistik konvensional seperti \textit{moving average} dan ARIMA memiliki keterbatasan dalam menangkap pola non-linear dan ketergantungan temporal jangka panjang pada data dengan karakteristik semacam ini.

Untuk mengatasi keterbatasan tersebut, pendekatan \textit{deep learning} berbasis \textit{Recurrent Neural Network} (RNN), khususnya \textit{Long Short-Term Memory} (LSTM), telah menjadi salah satu metode mutakhir dalam peramalan deret waktu penjualan. LSTM memanfaatkan mekanisme gerbang (\textit{gating mechanism}) untuk mempertahankan informasi lintas rentang waktu yang panjang sekaligus mengatasi masalah \textit{vanishing gradient} yang melekat pada RNN tradisional~[17]. Sejumlah penelitian terkini menunjukkan keunggulan pendekatan berbasis LSTM. Optimasi \textit{hyperparameter} menggunakan Keras Tuner pada LSTM untuk prediksi penjualan harian \textit{dataset} ritel Walmart menghasilkan peningkatan akurasi yang signifikan dibandingkan metode peramalan tradisional~[37]. De Castro Moraes dkk.~[38] mengembangkan arsitektur hibrida CNN-LSTM bertumpuk (\textit{stacked}) dan paralel untuk menangani pola musiman yang bervariasi serta korelasi antarproduk pada 500 deret waktu ritel. Sementara itu, Mansur dkk.~[36] menunjukkan bahwa model hibrida CNN-LSTM yang diperkaya variabel eksogen berupa hari libur, hari gajian, aksi unjuk rasa, dan kondisi cuaca mampu mencapai MAPE sebesar 4,16\%, mengungguli model ANN, CNN, dan LSTM tunggal. Perkembangan ini memperkuat posisi LSTM sebagai metode yang relevan untuk diintegrasikan ke dalam sistem POS.

Kendati demikian, terdapat dua kondisi yang membatasi keterbukaan hasil tersebut untuk direplikasi pada usaha berskala kecil. Pertama, model \textit{deep learning} bersifat \textit{data-hungry}, yaitu performanya sangat bergantung pada ukuran \textit{dataset}, sehingga pelatihan dengan data yang tidak memadai secara nyata membatasi kegunaannya~[39]. Beberapa penelitian yang melaporkan akurasi tinggi memang menggunakan data berskala besar, seperti~[38] yang memanfaatkan riwayat transaksi lima tahun. Kedua, ketika akurasi tinggi berhasil dicapai pada data berukuran kecil, keberhasilan tersebut umumnya bertumpu pada ketersediaan variabel eksogen yang kaya dan bukan pada panjang deret waktunya semata. Penelitian~[36] mencapai MAPE 4,16\% hanya dengan enam bulan data harian dari satu gerai, namun dengan tambahan tujuh variabel cuaca dan peristiwa, sementara penulisnya sendiri mencatat adanya indikasi \textit{overfitting} pada kurva \textit{validation loss}. Kondisi ini menyisakan persoalan bagi kasus di mana variabel eksogen semacam itu tidak tersedia. Sebagai jalan keluar, augmentasi data sintetis menggunakan \textit{Time-series Generative Adversarial Network} (TimeGAN)~[25] diusulkan untuk memperkaya sampel pelatihan dan terbukti menurunkan RMSE serta MAE pada beberapa domain~[40]. Namun literatur juga mencatat bahwa manfaat augmentasi semakin tidak signifikan seiring bertambahnya ukuran data awal~[41], sehingga efektivitas TimeGAN pada data penjualan harian berskala kecil masih merupakan pertanyaan terbuka yang layak diuji secara empiris.

Celah kedua bersifat metodologis. Sebagian besar penelitian peramalan penjualan berbasis LSTM membandingkan modelnya dengan sesama model \textit{machine learning}, tetapi jarang mengujinya secara ketat terhadap \textit{baseline} heuristik sederhana seperti \textit{seasonal-naive}, padahal pada data dengan musiman mingguan yang kuat \textit{baseline} semacam ini kerap sangat kompetitif. Pengalaman kompetisi peramalan berskala besar menunjukkan bahwa metode sederhana masih menjadi pembanding yang sulit dikalahkan dan karenanya wajib disertakan sebagai kontrol metodologis~[42]. Celah ketiga bersifat implementatif: kajian yang ada umumnya berhenti pada tahap eksperimen model dan belum sampai pada penyatuan model ke dalam aplikasi POS yang benar-benar digunakan pengguna akhir, sehingga jarak antara riset akademik dan penerapan praktis di ritel masih lebar~[43]. Penelitian ini menempatkan diri tepat pada ketiga celah tersebut, yaitu membangun aplikasi POS yang di dalamnya tertanam fitur prediksi penjualan harian berbasis LSTM, mengevaluasi kontribusi augmentasi TimeGAN, serta menguji model secara jujur terhadap \textit{baseline seasonal-naive}.
...
1 &
T. de Castro Moraes, X.-M. Yuan, E. P. Chew (2024) [38] &
...
5 &
J. Yoon, D. Jarrett, M. van der Schaar (2019) [25] &
...
6 &
P. Tang, Z. Li, X. Wang, X. Liu, P. Mou (2025) [40] &
...
7 &
A.-A. Semenoglou, E. Spiliotis, V. Assimakopoulos (2023) [41] &
...
8 &
S. Makridakis, E. Spiliotis, V. Assimakopoulos (2022) [42] &
...
9 &
R. A. Sembiring \& Y. Sary (2025), \textit{Jurnal Ilmiah Teknik Mesin, Elektro dan Komputer} [44] &
...
10 &
U. M. Jannah dkk. (2025), \textit{Jurnal Teknologi Informasi dan Multimedia} [45] &
...
11 &
R. Verdiyanto, D. Hartanti, E. Purwanto (2025), \textit{Infotek: Jurnal Informatika dan Teknologi} [46] &
...
\textit{Point of Sale} (POS) adalah sistem yang digunakan untuk mencatat, memproses, dan mendokumentasikan transaksi penjualan pada titik terjadinya transaksi. Dalam perkembangannya, aplikasi POS tidak lagi berfungsi semata sebagai mesin kasir digital, melainkan juga sebagai sistem informasi yang mengelola data produk, persediaan, pelanggan, dan pelaporan keuangan. Penerapan aplikasi POS pada usaha mikro, kecil, dan menengah terbukti membantu proses pencatatan transaksi menjadi lebih cepat, akurat, dan terdokumentasi, meskipun tingkat penerimaan penggunanya masih dipengaruhi oleh persepsi kemudahan dan kemanfaatan sistem [47]. Secara arsitektural, sistem POS umumnya terdiri atas modul autentikasi pengguna, modul manajemen produk, modul transaksi, modul persediaan, dan modul pelaporan [48].

Data transaksi yang dihasilkan aplikasi POS secara berkelanjutan memiliki sifat deret waktu, yakni terurut berdasarkan waktu dan memiliki ketergantungan antarpengamatan. Sifat inilah yang membuka peluang pemanfaatan data POS untuk keperluan peramalan, sehingga sistem tidak hanya bersifat \textit{descriptive} (mencatat apa yang telah terjadi) tetapi juga \textit{predictive} (memperkirakan apa yang akan terjadi).

\subsection{Peramalan Deret Waktu (\textit{Time Series Forecasting})}

Deret waktu adalah rangkaian pengamatan suatu variabel yang dicatat secara berurutan dalam interval waktu yang tetap. Peramalan deret waktu bertujuan memperkirakan nilai pengamatan pada periode mendatang berdasarkan pola yang terkandung pada pengamatan historisnya [20]. Secara umum, sebuah deret waktu dapat diuraikan menjadi empat komponen, yaitu tren (\textit{trend}), musiman (\textit{seasonality}), siklus (\textit{cyclical}), dan komponen tak beraturan (\textit{irregular} atau \textit{noise}).

Pada data penjualan harian kantin institusional, komponen musiman muncul dalam dua tingkatan. Musiman mingguan (periode $m = 7$) tampak dari perbedaan volume transaksi antara hari kerja dan akhir pekan, sedangkan musiman berbasis kalender akademik muncul dari perbedaan antara masa perkuliahan, minggu ujian, dan masa libur antarsemester.

Pendekatan peramalan secara garis besar terbagi menjadi tiga kelompok. \textit{Pertama}, pendekatan statistik klasik seperti \textit{moving average}, \textit{exponential smoothing}, ARIMA, dan SARIMA, yang mengasumsikan hubungan linear antarpengamatan [21]. \textit{Kedua}, pendekatan \textit{machine learning} seperti \textit{Random Forest}, \textit{Support Vector Regression}, dan \textit{Gradient Boosting}, yang memperlakukan peramalan sebagai persoalan regresi dengan fitur temporal yang direkayasa secara manual. \textit{Ketiga}, pendekatan \textit{deep learning} seperti RNN, LSTM, dan GRU, yang mampu mempelajari representasi temporal secara otomatis tanpa rekayasa fitur eksplisit.

\subsection{\textit{Baseline Seasonal-Naive}}

\textit{Seasonal-naive} adalah metode peramalan paling sederhana untuk data bermusiman, yang memperkirakan nilai suatu periode sama dengan nilai pada periode musiman terakhir yang bersesuaian [20]. Secara matematis, prediksi untuk waktu $t+h$ dirumuskan sebagai:
\begin{equation}
...
Meskipun sederhana, metode ini memiliki peran metodologis yang penting. Hyndman dan Koehler menegaskan bahwa metode naif berfungsi sebagai penyebut normalisasi dalam pengukuran akurasi peramalan, sehingga akurasi suatu model hanya bermakna apabila dinyatakan secara relatif terhadap metode naif tersebut [16]. Pengalaman kompetisi peramalan berskala besar juga menunjukkan bahwa metode sederhana kerap sulit dikalahkan oleh model kompleks, terutama pada deret waktu yang pendek atau sangat bergejolak [42]. Karena itu, penyertaan \textit{baseline seasonal-naive} dalam penelitian ini berfungsi sebagai kontrol metodologis untuk memastikan bahwa keunggulan model LSTM, apabila ada, benar-benar berasal dari kemampuan pemodelan dan bukan sekadar dari pola musiman yang telah dapat ditangkap oleh heuristik sederhana.

\subsection{\textit{Recurrent Neural Network} (RNN)}

\textit{Recurrent Neural Network} adalah arsitektur jaringan saraf tiruan yang dirancang untuk memproses data sekuensial dengan cara mempertahankan keadaan tersembunyi (\textit{hidden state}) yang diperbarui pada setiap langkah waktu. Pada langkah waktu $t$, keadaan tersembunyi dihitung sebagai:
\begin{equation}
...
Kelemahan utama RNN konvensional adalah masalah \textit{vanishing gradient}, yaitu mengecilnya nilai gradien secara eksponensial ketika dipropagasikan mundur melintasi banyak langkah waktu. Akibatnya, RNN kesulitan mempelajari ketergantungan jangka panjang [17].

\subsection{\textit{Long Short-Term Memory} (LSTM)}

\textit{Long Short-Term Memory} diperkenalkan oleh Hochreiter dan Schmidhuber sebagai pengembangan RNN yang mengatasi masalah \textit{vanishing gradient} melalui penambahan sel memori (\textit{cell state}) dan tiga mekanisme gerbang (\textit{gating mechanism}) [17]. Sel memori berfungsi sebagai jalur informasi yang memungkinkan gradien mengalir tanpa mengalami peluruhan berlebih, sedangkan gerbang mengendalikan informasi mana yang dibuang, disimpan, dan dikeluarkan.

\noindent\textbf{a. Gerbang Lupa (\textit{Forget Gate})}

Gerbang lupa menentukan proporsi informasi dari sel memori sebelumnya yang akan dipertahankan:
\begin{equation}
...
Gerbang masukan menentukan informasi baru yang akan disimpan ke dalam sel memori. Tahap ini terdiri atas dua bagian, yaitu penentuan proporsi pembaruan dan pembentukan kandidat nilai baru:
\begin{equation}
...
Sel memori diperbarui dengan menggabungkan informasi lama yang dipertahankan dan informasi baru yang diterima:
\begin{equation}
...
Gerbang keluaran menentukan bagian sel memori yang akan diteruskan sebagai keadaan tersembunyi:
\begin{equation}
...
Keempat komponen dilatih menggunakan kombinasi tiga fungsi galat, yaitu \textit{reconstruction loss}, \textit{unsupervised adversarial loss}, dan \textit{supervised loss}. Komponen \textit{supervised loss} inilah yang secara khusus memaksa \textit{generator} mempelajari transisi antarlangkah waktu, sehingga dinamika temporal data asli dapat dipertahankan pada data sintetis yang dihasilkan.

Dalam konteks penelitian ini, TimeGAN digunakan sebagai metode augmentasi data untuk memperbanyak sampel pelatihan pada \textit{dataset} yang berjumlah terbatas. Penerapan serupa telah dilaporkan berhasil menurunkan RMSE dan MAE pada domain konsumsi energi [40]. Namun demikian, perlu dicatat bahwa manfaat augmentasi tidak bersifat universal; kajian perbandingan sistematis menunjukkan bahwa keuntungan augmentasi cenderung menurun seiring bertambahnya ukuran data awal maupun berkurangnya kedalaman arsitektur jaringan [41].

\subsection{Praproses Data}

\noindent\textbf{a. Normalisasi}

Normalisasi diperlukan agar seluruh nilai masukan berada pada rentang yang seragam, sehingga mempercepat konvergensi pelatihan dan mencegah dominasi fitur bernilai besar. Metode yang umum digunakan pada LSTM adalah \textit{Min-Max Scaling}:
\begin{equation}
...
Data deret waktu univariat perlu diubah menjadi format pembelajaran terbimbing melalui teknik \textit{sliding window}. Dengan panjang jendela $w$, setiap sampel pelatihan terdiri atas masukan $[y_{t-w+1}, \dots, y_t]$ dan target $y_{t+1}$. Jumlah sampel yang terbentuk adalah $N - w$, dengan $N$ sebagai jumlah pengamatan. Konsekuensinya, semakin panjang jendela yang digunakan, semakin sedikit sampel pelatihan yang tersedia, yaitu pertimbangan yang menjadi kritis pada \textit{dataset} berukuran terbatas seperti pada penelitian ini.

\noindent\textbf{c. Pembagian Data}

Berbeda dengan data yang tidak berurutan, pembagian data deret waktu tidak boleh dilakukan secara acak karena akan menimbulkan kebocoran informasi masa depan (\textit{data leakage}). Pembagian dilakukan secara kronologis, dengan data terawal sebagai data latih dan data terakhir sebagai data uji [20].

\subsection{Metrik Evaluasi}

Evaluasi akurasi model peramalan dalam penelitian ini menggunakan metrik berikut, dengan $y_i$ sebagai nilai aktual, $\hat{y}_i$ sebagai nilai prediksi, dan $n$ sebagai jumlah pengamatan.
...
MAPE memudahkan interpretasi karena dinyatakan dalam persentase, namun tidak terdefinisi ketika terdapat nilai aktual sama dengan nol dan bersifat asimetris terhadap galat berlebih maupun kurang [16]. Keterbatasan ini relevan bagi penelitian ini mengingat \textit{dataset} memuat hari-hari tanpa transaksi.

\noindent\textbf{d. \textit{Mean Absolute Scaled Error} (MASE)}

Untuk mengatasi keterbatasan MAPE sekaligus memungkinkan perbandingan langsung terhadap \textit{baseline}, digunakan metrik MASE yang diusulkan Hyndman dan Koehler [16]:
\begin{equation}
...
& & Jupyter Notebook & Lingkungan \textit{pipeline} riset untuk eksplorasi data dan pelatihan model prediksi. \\ \cline{2-4}
...
\begin{enumerate}
    ...
    \item \textbf{Variabel Evaluasi:} Kinerja model diukur dengan empat metrik evaluasi (MAE, RMSE, sMAPE, dan MASE). MAPE konvensional dihindari karena menghasilkan nilai tak terhingga saat dihadapkan pada hari dengan pendapatan mendekati nol. Rincian formula masing-masing metrik dijabarkan pada Subbab 2.6.
\end{enumerate}
...
7 & Lainnya\footnote{Produk tanpa \texttt{category\_id} pada basis data transaksi (mis. produk yang telah dihapus dari katalog kategori namun riwayat transaksinya tetap tercatat).} & 73.607.992,00 & 19.454 & 14,68\% \\ \hline
\textbf{} & \textbf{Total} & \textbf{501.440.492,00} & \textbf{150.709} & \textbf{100,00\%} \\ \hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{\textit{raw\_transactions.csv}}: Berisi log transaksi gabungan hasil \textit{join} tabel \textit{invoices}, \textit{product\_sold}, \textit{products}, dan \textit{categories}.
    \item \textbf{\textit{daily\_sales.csv}}: Hasil agregasi harian yang menjadi dataset utama untuk model prediksi harian.
    \item \textbf{\textit{weekly\_sales.csv}}: Hasil agregasi mingguan untuk visualisasi tren mingguan.
    \item \textbf{\textit{monthly\_sales.csv}}: Hasil agregasi bulanan untuk analisis musiman jangka panjang.
\end{enumerate}

Struktur skema data dan deskripsi kolom dari berkas \textit{daily\_sales.csv} sebagai dataset utama pelatihan model LSTM dirinci pada Tabel \ref{tab:daily_sales_schema}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{Struktur Skema Dataset \textit{daily\_sales.csv}}
\label{tab:daily_sales_schema}
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\textbf{Nama Kolom} & \textbf{Tipe Data} & \textbf{Deskripsi} \\ \hline
\textit{date} & Date & Tanggal transaksi (format YYYY-MM-DD). \\ \hline
\textit{revenue} & Float & Total omzet pendapatan harian (nominal Rupiah). \\ \hline
\textit{transactions} & Integer & Total volume transaksi/nota lunas harian. \\ \hline
\textit{qty\_sold} & Integer & Total kuantitas unit produk terjual harian. \\ \hline
\textit{day\_of\_week} & Integer & Indeks hari dalam minggu (0 = Senin s.d. 6 = Minggu). Penyaringan hari kerja aktif (0 s.d. 4) dilakukan pada tahap pemodelan. \\ \hline
\textit{week\_of\_year} & Integer & Nomor minggu dalam setahun berdasarkan standar ISO. \\ \hline
\textit{month} & Integer & Nomor bulan kalender (1 s.d. 12). \\ \hline
\textit{is\_weekend} & Integer & Penanda biner akhir pekan (1 jika Sabtu/Minggu, 0 jika lainnya). \\ \hline
\textit{is\_holiday} & Integer & Penanda biner libur akademik (1 jika Januari/Juli, 0 jika aktif). \\ \hline
\textit{is\_ramadan} & Integer & Penanda biner bulan Ramadan (1 jika Ramadan, 0 jika lainnya). \\ \hline
\end{tabularx}
\end{table}

Sementara itu, skema data untuk berkas agregasi mingguan \textit{weekly\_sales.csv} dan agregasi bulanan \textit{monthly\_sales.csv} masing-masing dijelaskan pada Tabel \ref{tab:weekly_sales_schema} dan Tabel \ref{tab:monthly_sales_schema}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{Struktur Skema Dataset \textit{weekly\_sales.csv}}
\label{tab:weekly_sales_schema}
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\textbf{Nama Kolom} & \textbf{Tipe Data} & \textbf{Deskripsi} \\ \hline
\textit{year\_week} & String & Periode tahun dan nomor minggu ISO (format YYYY-WW). \\ \hline
\textit{week\_start} & Date & Tanggal awal hari Senin pada minggu tersebut (format YYYY-MM-DD). \\ \hline
\textit{revenue} & Float & Total omzet pendapatan mingguan (nominal Rupiah). \\ \hline
\textit{transactions} & Integer & Total volume transaksi/nota lunas mingguan. \\ \hline
\textit{qty\_sold} & Integer & Total kuantitas unit produk terjual mingguan. \\ \hline
\textit{active\_days} & Integer & Jumlah hari aktif beroperasi dalam minggu tersebut. \\ \hline
\end{tabularx}
\end{table}
...
\setstretch{1.2}
\centering
\caption{Struktur Skema Dataset \textit{monthly\_sales.csv}}
\label{tab:monthly_sales_schema}
\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\textbf{Nama Kolom} & \textbf{Tipe Data} & \textbf{Deskripsi} \\ \hline
\textit{year\_month} & String & Periode tahun dan bulan kalender (format YYYY-MM). \\ \hline
\textit{revenue} & Float & Total omzet pendapatan bulanan (nominal Rupiah). \\ \hline
\textit{transactions} & Integer & Total volume transaksi/nota lunas bulanan. \\ \hline
\textit{qty\_sold} & Integer & Total kuantitas unit produk terjual bulanan. \\ \hline
\textit{active\_days} & Integer & Jumlah hari aktif beroperasi dalam bulan tersebut. \\ \hline
\end{tabularx}
\end{table}
...
\begin{enumerate}
    \item \textbf{Volume Data Minimum (\textit{Cold Start Mitigation}):} Model LSTM memerlukan data runtun waktu yang kontinu. Penelitian ini membedakan dua ambang batas yang berbeda tujuan: (1) \textbf{7 hari aktif} merupakan syarat teknis minimum agar model dapat dijalankan sama sekali, sesuai panjang \textit{sequence} \texttt{LOOK\_BACK} pada arsitektur LSTM (lihat Subbab 3.2.7); dan (2) \textbf{30 hari aktif} merupakan ambang batas kualitas (\textit{warm-up period}) agar hasil prediksi dianggap cukup andal untuk ditampilkan tanpa peringatan \textit{cold start} kepada pengguna. Toko dengan riwayat transaksi kurang dari 7 hari tidak dapat memperoleh estimasi model sama sekali dan diberikan estimasi berbasis pola bisnis retail/kafe umum, sedangkan toko dengan riwayat antara 7--30 hari tetap memperoleh estimasi namun disertai peringatan \textit{cold start} bahwa keandalannya masih terbatas.
...
\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[36]} S. Mansur, K. Sattar, S. E. Hosseini, S. Pervez, I. Ahmad, K. Saleem, and A. Z. Elhendi, ``Sales Forecasting for Retail Stores Using Hybrid Neural Networks and Sales-Affecting Variables,'' \textit{PeerJ Computer Science}, vol. 11, e3058, 2025, doi: 10.7717/peerj-cs.3058.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[37]} M. Mitra, S. Roy, S. De, S. Bhattacharyya, J. Platos, and V. Snasel, ``Harnessing LSTM Neural Networks and Hyperparameter Optimization for Precise Sales Forecasting in Retail,'' in \textit{Proc. 10th International Conference on Advanced Intelligent Systems and Informatics (AISI 2024)}, Lecture Notes on Data Engineering and Communications Technologies, vol. 220. Cham, Switzerland: Springer, 2024, doi: 10.1007/978-3-031-71619-5\_11.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[38]} T. de Castro Moraes, X.-M. Yuan, and E. P. Chew, ``Hybrid Convolutional Long Short-Term Memory Models for Sales Forecasting in Retail,'' \textit{Journal of Forecasting}, vol. 43, no. 5, pp. 1278--1293, 2024, doi: 10.1002/for.3073.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[39]} ``Financial Time Series Augmentation Using Transformer Based GAN Architecture,'' \textit{arXiv preprint} arXiv:2602.17865, 2026. [\textit{Preprint}, belum melalui \textit{peer-review}].

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[40]} P. Tang, Z. Li, X. Wang, X. Liu, and P. Mou, ``Time Series Data Augmentation for Energy Consumption Data Based on Improved TimeGAN,'' \textit{Sensors}, vol. 25, no. 2, 2025, doi: 10.3390/s25020493.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[41]} A.-A. Semenoglou, E. Spiliotis, and V. Assimakopoulos, ``Data Augmentation for Univariate Time Series Forecasting with Neural Networks,'' \textit{Pattern Recognition}, vol. 134, art. no. 109132, 2023, doi: 10.1016/j.patcog.2022.109132.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[42]} S. Makridakis, E. Spiliotis, and V. Assimakopoulos, ``The M5 Competition: Background, Organization, and Implementation,'' \textit{International Journal of Forecasting}, vol. 38, no. 4, pp. 1325--1336, 2022, doi: 10.1016/j.ijforecast.2021.07.007.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[43]} P. Yadav and C. R. Barde, ``Retail Real-Time Sales Prediction System Using LSTM and XGBoost,'' \textit{International Journal of Advanced Research in Computer and Communication Engineering (IJARCCE)}, vol. 14, no. 4, 2025.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[44]} R. A. Sembiring and Y. Sary, ``Analisa dan Implementasi \textit{Long Short-Term Memory} (LSTM) dalam Kebutuhan Persediaan Barang di PT. Gunung Sari Indonesia,'' \textit{Jurnal Ilmiah Teknik Mesin, Elektro dan Komputer}, 2025.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[45]} U. M. Jannah dkk., ``Analisis Prediksi Penjualan Isi Ulang Air Galon Menggunakan Metode LSTM dan SARIMA,'' \textit{Jurnal Teknologi Informasi dan Multimedia}, 2025.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[46]} R. Verdiyanto, D. Hartanti, and E. Purwanto, ``Pengembangan Aplikasi \textit{Point of Sales} untuk Prediksi Penjualan Harian Usaha Minuman Menggunakan Algoritma \textit{Random Forest Regression},'' \textit{Infotek: Jurnal Informatika dan Teknologi}, vol. 8, no. 2, 2025.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[47]} E. M. Sipayung, C. Fiarni, and Wawan, ``Evaluasi Penggunaan Aplikasi \textit{Point of Sale} Menggunakan \textit{Technology Acceptance Model} pada UMKM,'' \textit{Jurnal Nasional Teknik Elektro dan Teknologi Informasi (JNTETI)}, vol. 9, no. 1, pp. 18--24, 2020, doi: 10.22146/jnteti.v9i1.116.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[48]} M. Siddik and S. Samsir, ``Rancang Bangun Sistem Informasi POS (\textit{Point of Sale}) untuk Kasir Menggunakan Konsep Bahasa Pemrograman Orientasi Objek,'' \textit{JOISIE (Journal of Information Systems and Informatics Engineering)}, vol. 4, no. 1, p. 43, 2020, doi: 10.35145/joisie.v4i1.607.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[49]} Google LLC, ``Flutter Documentation,'' 2026. [Daring]. Tersedia: https://docs.flutter.dev/ [Diakses: 29 Juli 2026].

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[50]} G. J. Myers, C. Sandler, and T. Badgett, \textit{The Art of Software Testing}, 3rd ed. Hoboken, NJ, USA: John Wiley \& Sons, 2011.

\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[51]} ISO/IEC/IEEE 29119-4:2021, \textit{Software and Systems Engineering --- Software Testing --- Part 4: Test Techniques}, International Organization for Standardization, Geneva, Switzerland, 2021.
