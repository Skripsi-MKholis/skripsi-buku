...
\begin{enumerate}
    ...
    \item \textbf{\texttt{daily\_sales.csv}}: Hasil agregasi harian yang menjadi dataset utama untuk model prediksi harian.
...
% Rancangan/Mermaid/activity_ai.png sebelum sidang.
    ...
    Gambar \ref{fig:act-produk} menggambarkan alur kerja staf toko dalam mengelola katalog produk yang menjadi sumber data transaksi bagi model prediksi penjualan. Proses tambah/ubah data disimpan terlebih dahulu ke basis data lokal Isar DB (\textit{offline-first}) beserta pencatatan riwayat perubahan stok, sebelum disinkronkan ke Supabase saat koneksi \textit{online} tersedia. Proses hapus produk diterapkan melalui skema \textit{soft delete} lokal (penanda \texttt{isDeleted=true}) yang baru dihapus permanen dari basis data setelah sinkronisasi ke \textit{cloud} berhasil, guna menjaga konsistensi data penjualan yang menjadi masukan pemodelan LSTM.
...
\begin{enumerate}
    ...
    \item Fungsi prediksi rekursif ({\small\texttt{forecast\_hybrid\_\allowbreak stable}}), untuk memverifikasi jalur eksekusi tiga mekanisme pengaman secara berurutan, yaitu \textit{clipping} \textit{log-return}, \textit{damping factor}, dan \textit{safety rail} nominal omzet.
\end{enumerate}
...
Cuplikan implementasi dua dari lima jaringan penyusun TimeGAN pada berkas \texttt{TimeGAN.py}, yaitu jaringan \textit{embedder} yang memetakan data riil ke ruang laten dan \textit{generator} yang memetakan derau acak menjadi representasi laten sintetis, disajikan pada Kode Program~\ref{lst:timegan}.
...
\hline
6 & {\small\texttt{forecast\_hybrid\_\allowbreak stable}} & Nilai \textit{log-return} mentah melebihi persentil 90 data latih (jalur \textit{clipping}) & Nilai \textit{log-return} dipotong tepat pada batas atas persentil 90 & Berhasil \\
\hline
7 & {\small\texttt{forecast\_hybrid\_\allowbreak stable}} & Iterasi horizon ke-$n$ dengan $n>1$ (jalur \textit{damping}) & Besaran perubahan \textit{log-return} teredam sesuai faktor $\phi^{(n-1)}$ & Berhasil \\
\hline
8 & {\small\texttt{forecast\_hybrid\_\allowbreak stable}} & Nilai omzet hasil rekonstruksi melampaui nilai maksimum historis (jalur \textit{safety rail}) & Nilai omzet dipotong tepat pada batas \texttt{REV\_CEIL} & Berhasil \\
\hline
\end{tabularx}
\end{table}
