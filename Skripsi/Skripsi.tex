...
\begin{enumerate}
    ...
    \item \textbf{Pelaku UMKM di Kota Lhokseumawe:} Sebagai subjek studi kasus untuk melakukan analisis kebutuhan pengguna secara langsung dan menguji kelayakan operasional aplikasi kasir di lapangan secara nyata.
\end{enumerate}
...
\begin{tabularx}{\textwidth}{|c|p{2.8cm}|p{3.5cm}|X|}
\hline
\textbf{No} & \textbf{Kategori} & \textbf{Nama Alat/Bahan} & \textbf{Spesifikasi / Deskripsi} \\ \hline
\multirow{3}{*}{1} & \multirow{3}{2.8cm}{Perangkat Keras (\textit{Hardware})} & Laptop Acer Nitro 5 & Intel Core i5 Gen 12, RAM 16 GB, SSD 512 GB, NVIDIA GeForce RTX 3050. Digunakan sebagai perangkat utama pengembangan aplikasi dan pelatihan model. \\ \cline{3-4}
& & Smartphone Android & OS Android 11. Digunakan untuk pengujian operasional aplikasi POS secara langsung di perangkat mobile. \\ \cline{3-4}
& & Printer Thermal & Bluetooth 58mm. Digunakan untuk menguji fungsionalitas cetak struk transaksi fisik. \\ \hline
\multirow{7}{*}{2} & \multirow{7}{2.8cm}{Perangkat Lunak (\textit{Software})} & Windows 11 & Sistem Operasi Laptop (Home Single Language). \\ \cline{3-4}
& & VS Code \& Android Studio & \textit{Integrated Development Environment} (IDE) utama untuk penulisan kode program. \\ \cline{3-4}
& & Flutter SDK (Dart) & \textit{Framework} lintas platform untuk membangun aplikasi kasir mobile. \\ \cline{3-4}
& & Python 3.10 & Bahasa pemrograman dengan TensorFlow, Keras, Pandas, NumPy, Scikit-Learn, dan FastAPI untuk melatih model LSTM dan membuat REST API. \\ \cline{3-4}
& & Supabase Cloud & Basis data \textit{cloud} terpusat (PostgreSQL) untuk sinkronisasi data transaksi. \\ \cline{3-4}
& & Isar Database & Basis data lokal terbenam (\textit{embedded}) pada perangkat Android (\textit{offline-first}). \\ \cline{3-4}
...
\texttt{day\_of\_week} & Integer & Indeks hari aktif (0 = Senin s.d. 4 = Jumat). \\ \hline
...
\begin{enumerate}
    ...
    \item \textbf{Keamanan Data (\textit{PII Scrubbing}):} Seluruh data transaksi yang ditarik dari database lokal (Isar DB) harus dibersihkan dari informasi sensitif pelanggan (seperti nama pelanggan, nomor telepon, dan detail pembayaran spesifik) sebelum dikirimkan ke cloud Supabase atau modul inferensi FastAPI di server untuk menjaga privasi.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        classbox/.style={
            ...
            minimum width=3.5cm, font=\small,
            align=left
        },
    ...
    \node (Category) [classbox, right=2cm of Store] {
        \textbf{Category}
        \nodepart{second}
        + supabaseId: String \\
        + storeId: String \\
    ...
    \node (Product) [classbox, below=1.5cm of Category] {
        \textbf{Product}
        \nodepart{second}
        + supabaseId: String \\
        + categoryId: String \\
    ...
    \node (Transaction) [classbox, below=1.5cm of Store] {
        \textbf{TransactionLocal}
        \nodepart{second}
        + supabaseId: String \\
        + totalAmount: Double \\
    ...
    \node (TransactionItem) [classbox, right=2cm of Transaction] {
        \textbf{TransactionItemLocal}
        \nodepart{second}
        + productId: String \\
        + quantity: Int \\
        + subtotal: Double
    ...
    % Connections
    \draw [arrow] (Store) -- (Category) node[midway, above] {1..*};
    \draw [arrow] (Category) -- (Product) node[midway, right] {1..*};
    \draw [arrow] (Store) -- (Transaction) node[midway, left] {1..*};
    \draw [arrow] (Transaction) -- (TransactionItem) node[midway, above] {1..*};
    \draw [arrow] (Product) -- (TransactionItem) node[midway, right] {1..*};
...
\begin{enumerate}
    ...
    \item \textit{Sync Engine} mendeteksi status internet aktif dan menyinkronkan transaksi ke cloud Supabase.
