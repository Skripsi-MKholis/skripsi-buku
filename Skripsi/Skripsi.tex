...
Secara umum, kebutuhan UMKM Indonesia -- termasuk pelaku usaha di Lhokseumawe sebagai salah satu ilustrasi konteks operasional daerah -- akan sistem kasir yang tidak hanya mampu mencatat transaksi, tetapi juga memprediksi penjualan harian secara akurat sekaligus tetap berfungsi dalam kondisi jaringan terbatas menjadi semakin penting. Kompleksitas pengambilan keputusan stok dan tingginya dinamika pola penjualan menuntut solusi yang memadukan keandalan operasional \textit{offline} dengan kecerdasan analitik prediktif dalam satu kesatuan sistem. Perlu ditegaskan bahwa data, observasi, dan pengujian pada penelitian ini seluruhnya dilaksanakan pada mitra EatsTEDI DTEDI Universitas Gadjah Mada, sehingga konteks UMKM Lhokseumawe di atas berfungsi sebagai motivasi umum permasalahan, bukan sebagai lokasi pengambilan data.
...
Perlu ditegaskan pula bahwa penelitian ini pada dasarnya bertujuan \textbf{mengevaluasi kelayakan (\textit{feasibility}) metode LSTM} dalam memprediksi penjualan harian pada volume data historis yang tersedia, bukan semata-mata mengklaim keunggulan LSTM tanpa pembuktian. Salah satu kontribusi utama penelitian ini justru adalah temuan berbasis bukti bahwa LSTM \textbf{belum layak} diterapkan pada volume data sebesar ini (313 hari aktif) dibandingkan metode \textit{baseline} statistik sederhana. Berdasarkan temuan tersebut, layanan prediksi produksi menerapkan mekanisme \textit{fallback} berbasis bukti berupa metode Seasonal-Naive ($k=4$), sementara model LSTM tetap didokumentasikan secara lengkap sebagai artefak riset. Arsitektur layanan prediksi (\textit{prediction service}) yang dibangun bersifat \textbf{model-agnostic}: \textit{endpoint} \texttt{/api/predict/daily} sudah disiapkan untuk dapat menyajikan model \texttt{.keras} begitu volume data transaksi mitra bertambah cukup untuk melampaui \textit{baseline} statistik secara konsisten.
