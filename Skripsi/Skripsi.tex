...
Data utama yang digunakan dalam penelitian ini adalah basis data transaksi penjualan harian yang bersumber dari platform katalog menu digital interaktif \textbf{EatsTEDI} pada Departemen Teknik Elektro dan Informatika (DTEDI/TRE) Universitas Gadjah Mada (UGM). Dataset ini berupa dump basis data relasional dalam format SQL (\texttt{eatstedi-20260621-010820.sql}) berukuran 32,15 MB dengan total sekitar 395.090 baris data.

Rentang waktu transaksi riil yang digunakan adalah dari tanggal \textbf{24 Agustus 2024 s.d. 20 Juni 2026} (mencakup periode 666 hari atau sekitar 22 bulan). Data yang diekstrak berfokus pada transaksi dengan status lunas (\texttt{is\_paid = 1}) dari tabel \texttt{invoices} serta rincian item produk terjual dari tabel \texttt{product\_sold} yang dihubungkan dengan tabel master \texttt{products} dan \texttt{categories}.
...
\begin{enumerate}
    \item \textbf{Pola Mingguan (Weekly Profile):} Penjualan aktif hampir 100\% terkonsentrasi pada hari kerja (Senin s.d. Jumat) dengan rata-rata 195,41 transaksi per hari aktif. Sedangkan pada hari Sabtu dan Minggu, transaksi bernilai Rp 0 karena kantin kampus tutup.
    \item \textbf{Pola Libur Akademis (Academic Holiday Profile):} Pada masa libur semester mahasiswa (sepanjang bulan Januari dan Juli), transaksi bernilai Rp 0 karena aktivitas perkuliahan dan operasional kantin berhenti total.
...
\begin{enumerate}
    ...
    \item \textbf{Dokumentasi Transaksi:} Mengunduh dan mengekstrak basis data transaksi digital EatsTEDI dari file dump SQL untuk tabel \texttt{invoices}, \texttt{product\_sold}, \texttt{products}, dan \texttt{categories} sebagai data latihan (\textit{training data}) dan data uji (\textit{testing data}) model prediksi LSTM.
...
\begin{enumerate}
    \item \textbf{Pembersihan Data (\textit{Data Cleaning} \& \textit{Imputation}):} Menghapus data duplikat dan menyelaraskan format tanggal. Untuk mengatasi gap/nilai kosong sebesar Rp 0 pada akhir pekan (Sabtu dan Minggu) serta libur semester panjang (Januari dan Juli), data disaring (\textit{filtering}) hanya menggunakan hari kerja aktif (\textit{business days}), serta dilakukan pengelompokan pelatihan terpisah per semester untuk menghindari bias prediksi akibat libur akademis yang panjang.
    ...
    \item \textbf{Pembentukan Struktur \textit{Time Series} (\textit{Windowing}):} Mengubah data sekuensial menjadi format input ($X$) dan label ($y$) dengan panjang jendela \textit{sliding window} tertentu, di mana data transaksi 14 hari kerja aktif sebelumnya digunakan untuk memprediksi penjualan hari aktif berikutnya.
    \item \textbf{Pembagian Data (\textit{Time-Based Split}):} Membagi dataset menjadi data latih (\textit{training data}) dan data uji (\textit{testing data}) menggunakan pemisahan berbasis waktu (\textit{time-based split}) untuk menjaga urutan temporal dan mencegah kebocoran data masa depan ke masa lalu.
    \item \textbf{Pelatihan Model:} Melatih arsitektur LSTM di sisi server menggunakan \textit{Adam Optimizer} dan fungsi aktivasi \textit{ReLU} pada lapisan tersembunyi untuk membangun model prediksi penjualan harian.
...
\begin{enumerate}
    \item \textbf{Variabel Input (Fitur):} Data deret waktu transaksi penjualan harian dari platform EatsTEDI yang mencakup:
    \begin{itemize}
        \item \textit{Total nilai transaksi harian} (nominal rupiah dari invoice lunas).
        \item \textit{Jumlah transaksi per hari} (volume invoice harian).
        \item \textit{Jumlah produk terjual per hari} (kuantitas unit barang terjual).
        \item \textit{Indeks hari dalam seminggu} (indeks integer 0 s.d. 4 untuk Senin--Jumat, dan 5--6 untuk Sabtu--Minggu).
        \item \textit{Status libur akademis} (fitur biner, bernilai 1 jika berada di bulan libur semester/Ramadan, dan 0 jika masa perkuliahan aktif).
    \end{itemize}
    \item \textbf{Variabel Output (Target):} Nilai prediksi total nominal penjualan harian untuk hari berikutnya, yang digunakan sebagai dasar pengambilan keputusan manajemen stok (terutama produk makanan basah) dan strategi operasional usaha.
    \item \textbf{Variabel Evaluasi:} Kinerja model diukur menggunakan tiga metrik evaluasi secara bersamaan, yaitu MAE (\textit{Mean Absolute Error}), RMSE (\textit{Root Mean Square Error}), dan MAPE (\textit{Mean Absolute Percentage Error}).
\end{enumerate}
