...
\subsection{Transformasi Target \textit{Log-Return} dan Rekonstruksi Nominal}

Pendekatan \textit{hybrid residual} digunakan agar model LSTM tidak memprediksi nilai pendapatan logaritmik $Y'_t$ secara langsung, melainkan memprediksi perubahan pendapatan relatif (\textit{log-return}) $r_t$, yang dirumuskan sebagai:
\begin{equation}
    r_t = Y'_t - Y'_{t-1} = \log(Y_t + 1) - \log(Y_{t-1} + 1)
\end{equation}
\noindent di mana $Y_t$ adalah nilai nominal pendapatan pada hari ke-$t$. Pendekatan ini bertujuan agar model mewarisi sifat autokorelasi jangka pendek yang dominan pada data penjualan.

Sebaliknya, keluaran model berupa \textit{log-return} prediksi ($\hat{r}_t$) direkonstruksi kembali ke skala nominal Rupiah ($\hat{Y}_t$) menggunakan nilai penjualan hari sebelumnya ($Y_{t-1}$), sebagai berikut:
\begin{equation}
    \hat{Y}_{t} = \exp(\log(Y_{t-1} + 1) + \hat{r}_{t}) - 1
\end{equation}

\section{Model Pembanding Prediksi (\textit{Baseline})}

Dalam penelitian prediksi deret waktu, kinerja sebuah model kompleks harus selalu disandingkan dengan model pembanding sederhana (\textit{baseline}) untuk memvalidasi apakah kompleksitas tambahan benar-benar memberikan nilai tambah akurasi [20]. Beberapa \textit{baseline} standar yang digunakan dalam penelitian ini adalah:
\begin{enumerate}
    \item \textbf{\textit{Naive}}: Nilai prediksi disamakan dengan nilai observasi terakhir yang tersedia. Metode ini menjadi tolok ukur minimum yang wajib dikalahkan oleh model yang lebih kompleks [20].
    \item \textbf{\textit{Seasonal-Naive}}: Nilai prediksi diambil dari observasi terakhir pada musim yang sama, misalnya penjualan hari Senin diprediksi dari rata-rata beberapa hari Senin sebelumnya, sehingga efektif menangkap pola musiman mingguan [20].
    \item \textbf{\textit{Moving Average} (Rata-rata Bergerak)}: Nilai prediksi berupa rata-rata dari $k$ observasi terakhir, berguna untuk meredam fluktuasi acak jangka pendek [14].
...
Temuan pada kompetisi prediksi berskala global M3 dan M4 menunjukkan bahwa metode statistik klasik dan kombinasinya kerap mengungguli metode \textit{machine learning} murni pada deret waktu bisnis, terutama ketika data historis yang tersedia terbatas [22],[23]. Temuan tersebut menjadi dasar metodologis dalam penelitian ini untuk mengevaluasi model LSTM secara jujur terhadap \textit{baseline} sederhana sebelum diputuskan metode mana yang layak disajikan kepada pengguna akhir.

\section{Metrik Evaluasi Prediksi (MAE, RMSE, sMAPE, dan MASE)}
...
\begin{enumerate}
    \item \textbf{\textit{Mean Absolute Error} (MAE):} Mengukur rata-rata selisih absolut antara nilai aktual penjualan $Y_t$ dengan nilai hasil prediksi $\hat{Y}_t$. Semakin kecil nilai MAE, maka model prediksi semakin akurat. Formula MAE didefinisikan sebagai berikut:
    \begin{equation}
        \text{MAE} = \frac{1}{n} \sum_{t=1}^{n} |Y_t - \hat{Y}_t|
    \end{equation}
    \noindent di mana $n$ adalah jumlah data pengujian.

    \item \textbf{\textit{Root Mean Square Error} (RMSE):} Memberikan bobot lebih besar pada kesalahan prediksi yang bernilai besar karena selisihnya dikuadratkan terlebih dahulu sebelum dirata-ratakan dan ditarik akarnya. Formula RMSE didefinisikan sebagai berikut:
    \begin{equation}
        \text{RMSE} = \sqrt{\frac{1}{n} \sum_{t=1}^{n} (Y_t - \hat{Y}_t)^2}
    \end{equation}

    \item \textbf{sMAPE} (\textit{Symmetric Mean Absolute Percentage Error}): Varian MAPE yang membagi galat absolut terhadap rata-rata nilai aktual dan prediksi, sehingga terhindar dari ledakan nilai persentase ketika nilai aktual mendekati nol dan memiliki batas atas galat sebesar 200\%. Formula sMAPE didefinisikan sebagai berikut:
    \begin{equation}
        \text{sMAPE} = \frac{1}{n} \sum_{t=1}^{n} \frac{|Y_t - \hat{Y}_t|}{(|Y_t| + |\hat{Y}_t|) / 2} \times 100\%
    \end{equation}
    Sebagai konvensi implementasi, apabila penyebut bernilai nol (nilai aktual dan prediksi sama-sama nol), penyebut diganti dengan nilai 1 agar tidak terjadi galat pembagian dengan nol (\textit{division by zero}).

    \item \textbf{MASE} (\textit{Mean Absolute Scaled Error}): Metrik yang diusulkan Hyndman dan Koehler [16] untuk menormalisasi galat model terhadap galat metode naif \textit{in-sample} pada data latih. Nilai MASE $< 1$ menandakan model lebih baik daripada metode naif, sehingga metrik ini dapat dibandingkan lintas dataset tanpa terpengaruh satuan nominal. Formula MASE didefinisikan sebagai berikut:
    \begin{equation}
        \text{MASE} = \frac{\frac{1}{n} \sum_{t=1}^{n} |Y_t - \hat{Y}_t|}{\frac{1}{N-1} \sum_{i=2}^{N} |Y^{\text{train}}_i - Y^{\text{train}}_{i-1}|}
    \end{equation}
    \noindent di mana $Y^{\text{train}}_i$ adalah nilai aktual pendapatan pada data latih, dan $N$ adalah jumlah data latih aktif.
\end{enumerate}
...
\vspace{0.3cm}
\noindent\hangindent=1.5cm
{[46]} R. Verdiyanto, D. Hartanti, and E. Purwanto, ``Pengembangan Aplikasi \textit{Point of Sales} untuk Prediksi Penjualan Harian Usaha Minuman Menggunakan Algoritma \textit{Random Forest Regression},'' \textit{Infotek: Jurnal Informatika dan Teknologi}, 2025.
