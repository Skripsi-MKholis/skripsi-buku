...
Modul kecerdasan buatan untuk melakukan kalkulasi prediksi penjualan harian dikembangkan menggunakan Python dengan library TensorFlow/Keras, lalu di-\textit{deploy} sebagai layanan inferensi pada platform Hugging Face. Modul ini beroperasi secara terpisah dari beban kerja transaksional POS untuk menjaga stabilitas aplikasi kasir.

Proses operasional modul AI berjalan melalui alur kerja sebagai berikut:
\begin{enumerate}
    \item \textbf{Agregasi Data di Sisi Klien}: Aplikasi Flutter mengagregasi data transaksi penjualan harian dari basis data lokal Isar DB menjadi \textit{payload} deret waktu agregat (tanpa informasi identitas pelanggan) pada saat pengguna membuka fitur \textit{Smart Analytics}.
    \item \textbf{Pra-pemrosesan Data}: Di sisi server inferensi, data penjualan harian dibersihkan dari nilai kosong menggunakan interpolasi linier dan dinormalisasi ke skala 0 hingga 1 menggunakan \textit{MinMaxScaler} sebelum diumpankan ke model LSTM.
    \item \textbf{FastAPI REST Endpoints}: Layanan inferensi menyediakan endpoint API berbasis REST (seperti \textit{/predict}) menggunakan framework FastAPI. Endpoint ini dipanggil langsung oleh aplikasi Flutter melalui HTTPS dan mengembalikan respons berupa deret angka prediksi penjualan harian untuk hari-hari berikutnya. Hasil analisis kemudian disimpan aplikasi sebagai \textit{snapshot} pada tabel \textit{smart\_analytics\_snapshots} di Supabase.
\end{enumerate}
