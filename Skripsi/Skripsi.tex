...
\noindent 4)~~\textit{Use Case} Menerapkan \textit{Voucher}/Diskon

Rancangan \textit{use case} menerapkan \textit{voucher} atau diskon digunakan untuk memberikan potongan harga pada transaksi yang sedang berjalan. Rancangan \textit{use case} ini dapat dilihat pada Tabel~\ref{tab:uc-diskon}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{\textit{Use Case} Menerapkan \textit{Voucher}/Diskon}
\label{tab:uc-diskon}
\begin{tabularx}{\textwidth}{|p{3.2cm}|X|}
\hline
\textbf{\textit{Use Case}} & \textbf{Menerapkan \textit{Voucher}/Diskon} \\ \hline
Deskripsi & Proses pemberian potongan harga pada total tagihan transaksi \\ \hline
\textit{Actor} & Owner dan Kasir \\ \hline
\textit{Precondition} & 1.~Terdapat transaksi aktif pada keranjang \newline 2.~Pengguna memasukkan kode promosi yang berlaku \\ \hline
\textit{Postcondition} & Total tagihan transaksi terpotong sesuai nilai diskon \\ \hline
\end{tabularx}
\end{table}

Tabel~\ref{tab:uc-diskon} menunjukkan bahwa sistem dapat menyesuaikan nilai tagihan berdasarkan potongan harga yang diterapkan.

\noindent 5)~~\textit{Use Case} Melakukan \textit{Split Bill}

Rancangan \textit{use case} melakukan \textit{split bill} digunakan untuk memecah satu nota belanja menjadi beberapa bagian tagihan yang dibayarkan secara terpisah. Rancangan \textit{use case} ini dapat dilihat pada Tabel~\ref{tab:uc-splitbill}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{\textit{Use Case} Melakukan \textit{Split Bill}}
\label{tab:uc-splitbill}
\begin{tabularx}{\textwidth}{|p{3.2cm}|X|}
\hline
\textbf{\textit{Use Case}} & \textbf{Melakukan \textit{Split Bill}} \\ \hline
Deskripsi & Proses pemecahan satu nota belanja menjadi beberapa bagian tagihan \\ \hline
\textit{Actor} & Owner dan Kasir \\ \hline
\textit{Precondition} & 1.~Terdapat transaksi aktif pada keranjang \newline 2.~Pengguna memilih menu pemecahan tagihan \\ \hline
\textit{Postcondition} & Tagihan terbagi dan masing-masing bagian dapat diselesaikan pembayarannya \\ \hline
\end{tabularx}
\end{table}

Tabel~\ref{tab:uc-splitbill} menunjukkan bahwa sistem mengakomodasi pembayaran terpisah dalam satu nota belanja.

\noindent 6)~~\textit{Use Case} Mencetak Struk

Rancangan \textit{use case} mencetak struk digunakan untuk menghasilkan bukti transaksi fisik bagi pelanggan setelah pembayaran diselesaikan. Rancangan \textit{use case} ini dapat dilihat pada Tabel~\ref{tab:uc-struk}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{\textit{Use Case} Mencetak Struk}
\label{tab:uc-struk}
\begin{tabularx}{\textwidth}{|p{3.2cm}|X|}
\hline
\textbf{\textit{Use Case}} & \textbf{Mencetak Struk} \\ \hline
Deskripsi & Proses pencetakan bukti transaksi melalui \textit{printer thermal} \\ \hline
\textit{Actor} & Owner dan Kasir \\ \hline
\textit{Precondition} & 1.~Transaksi telah diselesaikan pembayarannya \newline 2.~Perangkat \textit{printer thermal} telah terhubung \\ \hline
\textit{Postcondition} & Struk transaksi berhasil tercetak \\ \hline
\end{tabularx}
\end{table}

Tabel~\ref{tab:uc-struk} menunjukkan bahwa sistem menyediakan bukti transaksi tercetak bagi pelanggan.

\noindent 7)~~\textit{Use Case} Memonitor Status Meja

Rancangan \textit{use case} memonitor status meja digunakan untuk memantau keterisian meja secara visual selama layanan berlangsung. Rancangan \textit{use case} ini dapat dilihat pada Tabel~\ref{tab:uc-meja}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{\textit{Use Case} Memonitor Status Meja}
\label{tab:uc-meja}
\begin{tabularx}{\textwidth}{|p{3.2cm}|X|}
\hline
\textbf{\textit{Use Case}} & \textbf{Memonitor Status Meja} \\ \hline
Deskripsi & Proses pemantauan status keterisian meja pada alur layanan \\ \hline
\textit{Actor} & Owner dan Kasir \\ \hline
\textit{Precondition} & 1.~Pengguna telah masuk ke dalam sistem \newline 2.~Pengguna membuka tampilan status meja pada halaman kasir \\ \hline
\textit{Postcondition} & Status keterisian meja tersaji secara terbarui \\ \hline
\end{tabularx}
\end{table}

Tabel~\ref{tab:uc-meja} menunjukkan bahwa pengguna dapat mengetahui ketersediaan meja tanpa keluar dari alur transaksi.

\noindent 8)~~\textit{Use Case} Mengelola Katalog Produk
...
\noindent 9)~~\textit{Use Case} Mengelola Kategori
...
\noindent 10)~~\textit{Use Case} Mengaudit Riwayat Stok
...
\noindent 11)~~\textit{Use Case} Melihat \textit{Dashboard}
...
\noindent 12)~~\textit{Use Case} Melihat AI \textit{Smart Analytics}

Rancangan \textit{use case} melihat AI \textit{Smart Analytics} digunakan untuk menyajikan hasil prediksi penjualan harian yang dihasilkan model \textit{Long Short-Term Memory} melalui layanan \textit{REST API}. Rancangan \textit{use case} ini dapat dilihat pada Tabel~\ref{tab:uc-analytics}.

\begin{table}[H]
\setstretch{1.2}
\centering
\caption{\textit{Use Case} Melihat AI \textit{Smart Analytics}}
\label{tab:uc-analytics}
\begin{tabularx}{\textwidth}{|p{3.2cm}|X|}
\hline
\textbf{\textit{Use Case}} & \textbf{Melihat AI \textit{Smart Analytics}} \\ \hline
Deskripsi & Proses penyajian hasil prediksi penjualan harian beserta riwayat analisisnya \\ \hline
\textit{Actor} & Owner dan Kasir \\ \hline
\textit{Precondition} & 1.~Pengguna telah masuk ke dalam sistem \newline 2.~Riwayat transaksi telah memenuhi panjang sekuens minimum yang disyaratkan model \\ \hline
\textit{Postcondition} & Hasil prediksi penjualan harian tersaji dan tersimpan pada riwayat analisis \\ \hline
\end{tabularx}
\end{table}

Tabel~\ref{tab:uc-analytics} menunjukkan bahwa hasil peramalan dapat langsung dimanfaatkan pengelola di dalam aplikasi yang sama.

\noindent 13)~~\textit{Use Case} Kirim \textit{Broadcast} Notifikasi
...
\noindent 14)~~\textit{Use Case} Sinkronisasi Data
...
\noindent 15)~~\textit{Use Case} Mengelola Karyawan
...
\noindent 16)~~\textit{Use Case} Mengatur Profil Toko
