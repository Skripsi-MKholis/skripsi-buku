...
\begin{enumerate}
    \item Kasir memasukkan transaksi penjualan melalui antarmuka aplikasi POS (\textit{inputTransaksi()}).
    \item Aplikasi menyimpan data transaksi secara lokal ke dalam Isar DB dengan status belum tersinkronisasi (\textit{writeTransaction(isSynced=false)}).
    ...
    \item \textit{Sync Engine} mendeteksi status internet aktif dan menyinkronkan transaksi ke cloud Supabase melalui pemanggilan RPC \textit{create\_transaction\_v4}.
    \item Supabase mengembalikan respon sukses, lalu \textit{Sync Engine} memperbarui status transaksi lokal menjadi \textit{isSynced=true}.
\end{enumerate}
